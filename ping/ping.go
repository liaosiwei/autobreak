...
func Ping(address string, timeout int) bool {
	_, err := Pinger(address, timeout)
	return err == nil
}

func Pinger(address string, timeout int) (float64, error) {
	...
	wb, err := (&icmpMessage{
		...
		Body: &icmpEcho{
			ID: xid, Seq: xseq,
			Data: bytes.Repeat([]byte("Go Ping!!!"), 1),
		},
	}).Marshal()
	if err != nil {
		return 0, err
	...
	dur := math.Trunc(float64(end.Sub(start).Nanoseconds()) / 1000)
	return float64(dur) / 1000, nil
}
